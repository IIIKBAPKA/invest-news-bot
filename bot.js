...
// Парсер 1: Маскування під браузер для Seeking Alpha та Yahoo
...
// Додаємо Yahoo Finance у базовий масив фідів замість WSJ
const FEEDS = [
    ...
    { name: 'YahooFinance', url: 'https://finance.yahoo.com/news/rss' }
];
...
async function run() {
    try {
        console.log("🚀 Запуск моніторингу ( Seeking Alpha + Yahoo Finance + SEC | Інтервал: 16 хв)...");
        let allItems = [];
        let sourceStats = { SeekingAlpha: 0, SEC: 0, YahooFinance: 0 };

        for (const feedSource of FEEDS) {
            try {
                // Вибір правильного парсера
                const activeParser = feedSource.name === 'SEC' ? parserSEC : parserBrowser;
        ...
        let passedBySource = { SeekingAlpha: 0, SEC: 0, YahooFinance: 0 };
        ...
        console.log(`- Пройшли фільтр (16хв + Тікер/Назва):`);
        ...
        for (const item of uniqueItems) {
            ...
            // Пробуємо Jina для SA та Yahoo
            if (item.sourceName === 'SeekingAlpha' || item.sourceName === 'YahooFinance') {
                try {
            ...
            🎯 <b>Головне:</b> [Зроби тут як новинний заголовок ро що вона]
